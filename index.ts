...
const runGivenCode = async (code: string, driver: WebDriver): Promise<string> => {
	...
	const approved = await approve(runnableCode, (e) => e, driver, Key, until)
	if(approved.approved) return JSON.stringify(approved.return)
	else return '[Code has been denied.]'
}
